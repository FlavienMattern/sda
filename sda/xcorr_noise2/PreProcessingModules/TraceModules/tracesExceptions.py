...
class ErrorParamFromParamFile(ErrorParam):
    ...
    def __str__(self):
        ...
        if self.NameArgument == 'badOrderDates':
            self.__Message = "Error for parameters date: the first date is greater than the last date in the file:" +  tracesParam.__file__
        if self.NameArgument == 'ListOfStations':
            self.__Message = "There is no list of stations named \'ListOfStations\' define in the module named: " + tracesParam.__file__
            self.__Message += "\nOr the list of station \'ListOfStations\' is empty,"
        if self.NameArgument == 'FormatTraceSave':
            self.__Message = "There is no string named \'FormatTrace\' in the module named: " + tracesParam.__file__
            self.__Message += "\nOr the format is not exactly \'mat\' or \'npy\'"
        if self.NameArgument == 'LoadDirectory':
            self.__Message = "There is no string named \'LoadDirectory\' in the module named: " + tracesParam.__file__
            self.__Message += "\nOr the LoadDirectory do not exist or cannot be read."
        if self.NameArgument == 'SaveDirectory':
            self.__Message = "There is no string named \'SaveDirectory\' in the module named: " + tracesParam.__file__
            self.__Message += "\nOr the SaveDirectory cannot be write."
        if self.NameArgument == 'ComponentStation':
            self.__Message = "There is no string named \'ComponentStation\' in the module named: " + tracesParam.__file__
            self.__Message += "\nOr the ComponentStation is not a letter in \'E\', \'N\', \'Z\'."
        if self.NameArgument == 'NewFrequence':
            self.__Message = "There is no float named \'NewFrequence\' in the module named: " + tracesParam.__file__
