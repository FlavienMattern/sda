...
def Stack(time, lagtime, data, sta1, sta2, comp, config):
    ...
    if np.count_nonzero(~np.isnan(StackDict["array"])) != 0:        
        ...
        filename = os.path.join(savepath,"{}-{}.csv".format(sta1,sta2))
        ...
        df = pd.DataFrame(data=StackDict["array"].T, index=pd.to_datetime(StackDict["timeCenter"]), columns=StackDict["lagtime"])
        df.to_csv(filename)