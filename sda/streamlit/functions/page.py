			...
			for itab, tab_id in enumerate(tab_list[:-1]):
				...
				with tab:
					...
					if render_mode == "edition":
						if st.button(":material/splitscreen_add:", key=f"{tab_id}_new_line", use_container_width=True):
	...
	def create_page(self, title, visible=True, removable=True, default_page=False):
		...
		st.session_state["session"]["content"]["pages"][page_id] = {
			...
			"tabs": {
				"tab_000": {
					"name": "Tab 1",
					"layout": [[{"id": "block_1", "module":None, "sub_blocks": []}]]
				}
			}
		}
	...
	def render_block(self, block, parent_blocks, idx, key_prefix, render_mode, tab_id):
		...
		with st.container(border=True):
			if render_mode == "edition":
				...
				col_add_line, _, col_add_right, _ = st.columns([0.4, 0.1, 0.4, 0.11])

				
				# Ajouter une nouvelle ligne avec un seul bloc dans sub_blocks
				if col_add_line.button(":material/desktop_landscape_add:", key=f"{key_prefix}_add_row", use_container_width=True):
					new_id = generate_unique_id()
