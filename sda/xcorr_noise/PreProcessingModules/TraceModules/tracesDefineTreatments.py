...
from sda.xcorr_noise.PreProcessingModules.TraceModules import tracesFunctions
import obspy
import time
import numpy as np
...
def makeTreatmentTraceAfterNewFrequence(Trace, Frequence, acorr, remove_response, response_dict, config):#Trace is destroyed
    ...
    config["response_dict"] = response_dict
    ...
    # import os
    # foldername = os.path.join("/media/flavien/WORK/these/schema/RESULTS/workflow_data", response_dict["station_name"].split(".")[1], response_dict["start"].strftime("%Y-%m-%d"))
    # filename = os.path.join(foldername, f"resp_resampling_highpass_data.pkl")
    # print(filename)
    ...
    # import os
    # foldername = os.path.join("/media/flavien/WORK/these/schema/RESULTS/workflow_data", response_dict["station_name"].split(".")[1], response_dict["start"].strftime("%Y-%m-%d"))
    # filename = os.path.join(foldername, f"glitch_correction.pkl")
    # print(filename)
    # with open(filename, "wb") as f:
    #     pkl.dump(Trace, f)
    #############################
